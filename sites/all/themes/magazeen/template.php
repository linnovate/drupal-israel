<?php
// $Id: template.php,v 1.6.2.2.2.1 2009/06/26 22:02:57 zinv Exp $
...
if (is_null(theme_get_setting('user_notverified_display'))) {
  ...
  $defaults = array(
    ...
    'comment_field_name'                    => t('Name'),
    'comment_field_email'                   => t('Mail (will not be published) '),
    'comment_field_website'                 => t('Website'),
    'comment_field_subject'                 => t('Subject'),
  ...
  foreach ($node_types as $type => $name) {
    ...
    $defaults["comment_add_{$type}"]              = t($defaults['comment_add_default']);
...
function magazeen_preprocess_page(&$vars) {
...
function magazeen_preprocess_node(&$vars) {
...
//theme_get_setting('comment_new_title_'. $node_content_type)), 
 ...
 */
function magazeen_preprocess_search_result(&$variables) {
 ...
 */
function magazeen_preprocess_comment(&$vars, $hook) {
 ...
 */
function magazeen_username($object) {
...
function magazeen_search_theme_form($form) {
 ...
 * Overwrite RSS feed.
 */
function magazeen_feed_icon($url) {
 ...
 */
function magazeen_comment_wrapper($content, $node) {
  $comment_count = format_plural($node->comment_count, '1 Comment', '@count Comments');
  $no_comment_message = t(theme_get_setting('no_comment_message'));
...
function magazeen_comment_form($form) {
  $output = '';
    ...
    if (isset($form['_author'])) {
      ...
      $form['_author']['#value'] = '<p>'.t('Logged in as !user. !logout', array('!user' => $form['_author']['#value'],'!logout' => l('Log out »', 'logout'))).'</p>';
    }
  ...
  $form['name']['#title'] = theme_get_setting('comment_field_name');
  $form['mail']['#title'] = theme_get_setting('comment_field_email');
  $form['homepage']['#title'] = theme_get_setting('comment_field_website');
  $form['subject']['#title'] = theme_get_setting('comment_field_subject');
...
function magazeen_form_element($element, $value) {
  if (empty($element['#comments_element'])) {
    ...
    if (theme_get_setting('display_description') == 1) {
